        ...
        if chemical_system_mode == "exact":
            ...
            if not torch.all(do_not_mask_atom_logits):
                ...
                if torch.any(atoms_to_bias):
                    ...
                    # Count the number of elements in the chemical system to balance bias
                    num_specified_elements = chemical_system_bias.sum(dim=1, keepdim=True).clamp(min=1)
                    ...
                    bias_tensor[atoms_to_bias] = chemical_system_bias[batch_indices_for_biased_atoms] * base_bias_strength[batch_indices_for_biased_atoms]
